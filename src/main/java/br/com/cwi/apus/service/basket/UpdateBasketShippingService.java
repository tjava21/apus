...
@AllArgsConstructor
@Service
public class UpdateBasketShippingService {
    ...
    @Transactional
    public void execute(String id, UpdateBasketShippingRequest request) {

        Basket basket = findBasketByIdService.getDomain(id);

        basket.setZip(request.getZip());
        basket.setAddress(request.getAddress());
